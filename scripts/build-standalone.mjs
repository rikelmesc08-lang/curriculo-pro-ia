...
// `--webpack` PELO MESMO MOTIVO DO `npm run build`, e precisa ser repetido
// aqui: o servidor da Hostinger tem glibc antigo, não carrega o SWC nativo e
// recusa o Turbopack ("Turbopack is not supported on this platform (linux/x64)
// because native bindings are not available"). Este script chama o `next build`
// por conta própria, então não herda a flag do script de `package.json` — sem
// ela, o empacotamento standalone seria o único caminho de compilação do
// projeto que quebra exatamente no servidor para o qual ele existe.
